...
class TaskManagerGUI:
    def __init__(self, root):
        ...
        self.plot_frame.grid(row=0, column=10, rowspan=5, sticky='nsew')

        # Initialize the plot
        self.figure = plt.Figure(figsize=(20, 18), dpi=75)
    ...
    def plot_tasks(self, tasks):
        ...
        for i, task in enumerate(sorted(tasks, key=lambda x: x.start_time)):
            ...
            for j, subtask in enumerate(task.subtasks):
                ...
                self.ax.barh(i, end_num - start_num, left=start_num, height=0.8, color=shade, edgecolor='blue')
                self.ax.text(start_num, i, f"{subtask.description}", horizontalalignment='left', verticalalignment='center', rotation=270)
